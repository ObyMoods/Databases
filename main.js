...
async function handleDelSrvCommand(client, chatId, message, textArgs, prefix) {
  ...
  try {
    ...
    await client.sendMessage(chatId, { message: `<blockquote>✅ Server ID ${serverId} berhasil dihapus</blockquote>`, parseMode: "html", parseMode: "html", replyTo: message.id });
  } catch (err) {
...
╰──────────────\n\n© ʙʏ ᴋɪɴɢs 👑
  ...
  (async () => {
    try {
      ...
      if (!text) {
        await client.sendMessage(chatId, {
          message: `<blockquote>⚠️ Prefix saat ini: "${prefix}"</blockquote>`,
          parseMode: "html",
          replyTo: message.id
        });
        return;
      ...
      if (text === '""' || text === '"') {
        ...
        await client.sendMessage(chatId, {
          message: `<blockquote>✅ Prefix diubah <b>tanpa simbol</b></blockquote>`,
          parseMode: "html",
          replyTo: message.id
        });
        return;
      ...
      await client.sendMessage(chatId, {
        message: `<blockquote>✅ Prefix berhasil diubah ke: "${text}"</blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
  ...
  (async () => {
    try {
      ...
      if (!targetUser && text) {
        try {
          ...
          return client.sendMessage(chatId, {
            message: "<blockquote><b>❌ Username atau ID tidak valid</b></blockquote>",
            parseMode: "html",
            replyTo: message.id
          });
        }
      ...
      if (!targetUser) targetUser = await client.getEntity('me');
...
Bio: ${fullUser.fullUser.about || 'No description'}
  ...
  (async () => {
    try {
      ...
      if (!text) {
        return client.sendMessage(chatId, {
          message: `<blockquote>⚠️ Gunakan: ${prefix}xn <kata kunci></blockquote>`,
          parseMode: "html",
          replyTo: message.id
        });
      }
      const loadingMsg = await client.sendMessage(chatId, {
        message: `<blockquote>⏳ Mencari video...\n🔍 Kata kunci: <b>${escapeHtml(text)}</b></blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
      ...
      if (!msgId) {
        console.log("⚠️ Tidak bisa mengambil ID pesan, abort edit.");
        return;
      }
      const safeEdit = async (msgText) => {
        try {
          ...
          if (!/FLOOD_WAIT|Too Many Requests/i.test(err.message)) {
            console.log("⚠️ Edit gagal (skip):", err.message);
          }
        }
      };
      const safeDelete = async (msgObj) => {
        try {
          ...
          console.log("⚠️ Gagal delete message:", err.message);
        }
      };
      await safeEdit(`🔍 <b>Mencari video...</b>\n<b>Kata kunci:</b> ${escapeHtml(text)}`);
      const res = await fetch(`https://restapi-v2.simplebot.my.id/search/xnxx?q=${encodeURIComponent(text)}`);
      const data = await res.json();

      if (!data.status || !data.result?.length) {
        return safeEdit(`⚠️ Tidak ada hasil ditemukan untuk: <b>${escapeHtml(text)}</b>`);
      }

      const top = data.result[0];
      const title = top.title || text;
      const link = top.link;
      await safeEdit(`⌛ <b>Mengambil detail video...</b>\n🎬 Judul: <b>${escapeHtml(title)}</b>`);
      const dlRes = await fetch(`https://restapi-v2.simplebot.my.id/download/xnxx?url=${encodeURIComponent(link)}`);
      const dlData = await dlRes.json();
      const result = dlData.result;
      const high = result.files?.high;

      if (!high) {
        return safeEdit(`⚠️ Video tidak memiliki kualitas High (HD).\n\n<b>Judul:</b> ${escapeHtml(title)}`);
      }
      await safeEdit(`📦 <b>Mengunduh video...</b>\n🎞 Resolusi: High + THANKS TO XLILNYX`);
      const videoRes = await fetch(high);
      const buffer = Buffer.from(await videoRes.arrayBuffer());
      const filePath = path.join(process.cwd(), `temp_${Date.now()}.mp4`);
      fs.writeFileSync(filePath, buffer);

      await safeEdit(`✅ <b>Video ditemukan!</b>\n📤 Mengirim ke chat...`);
      await safeDelete(loadingMsg);
      await client.sendMessage(chatId, {
        file: filePath,
        caption: `
<blockquote><b>🎬 HASIL VIDEO</b>
────────────────────────────
...
© ʙʏ ᴋɪɴɢs 👑
      ...
      await client.sendMessage(chatId, {
        message: `<blockquote><b>Video berhasil dikirim!</b>\n<b>© ʙʏ ᴋɪɴɢs 👑</b></blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
      ...
      await client.sendMessage(message.chat?.id || message.chatId, {
        message: `<blockquote>❌ Terjadi kesalahan saat mengambil data</blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
    }
...
case "cfdgroup": {
  (async () => {
    try {
      ...
      if (!repliedId) {
        return await client.sendMessage(chatId, {
          message: `<blockquote>⚠️ <b>Balas pesan</b> yang ingin dikirim ke semua grup</blockquote>`,
          parseMode: "html",
          replyTo: message.id,
        });
      }
  ...
  (async () => {
    try {
      ...
      if (tokenBaru) {
        await client.sendMessage(chatId, {
          message:
            `<blockquote>✅ <b>Token baru berhasil diambil!</b>\n🤖 Bot: @${botUsername}\n🔑 Token Baru:</blockquote> <code>${tokenBaru}</code></blockquote>`,
          parseMode: "html",
          replyTo: message.id
        });
      } else {
...
case 'tohd': {
  (async () => {
    try {
      ...
      try { await client.deleteMessages(chatId, [loading.id], { revoke: true }); } catch {}
  ...
  (async () => {
    try {
      ...
      if (!message.isGroup && !message.isSuperGroup && !message.isChannel) {
        return client.sendMessage(chatId, {
          message: "<blockquote>⚠️ Perintah ini hanya bisa di grup</blockquote>",
          parseMode: "html",
          replyTo: message.id
        });
      }
      ...
      try {
        loading = await client.sendMessage(chatId, {
          message: "<blockquote>🔍 Mencari akun terhapus...</blockquote>",
          parseMode: "html",
          replyTo: message.id
        });
      } catch {
      ...
      try {
        ...
        return client.sendMessage(chatId, {
          message: `<blockquote>❌ Gagal mengambil peserta grup: ${e.message}</blockquote>`,
          parseMode: "html",
          replyTo: message.id
        });
      }
      ...
      if (!zombies.length) {
        ...
        return client.sendMessage(chatId, {
          message: "<blockquote>✅ Tidak ada akun terhapus di grup ini</blockquote>",
          parseMode: "html",
          replyTo: message.id
        });
      }
      ...
      await client.sendMessage(chatId, {
        message: `<blockquote>✅ Pembersihan selesai\n\n🧟‍♂️ Total zombie: <b>${zombies.length}</b>\n✅ Dikeluarkan: <b>${removed}</b>\n❌ Gagal: <b>${failed}</b></blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
      ...
      await client.sendMessage(message.chat?.id || message.chatId, {
        message: `<blockquote>❌ Terjadi kesalahan: <code>${String(err.message)}</code></blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
    }
...
case "promote": {
  (async () => {
    try {
      ...
      if (!targetId) {
        return client.sendMessage(chatId, {
          message: "<blockquote>❌ Tidak menemukan kacung</blockquote>",
          parseMode: "html",
          replyTo: message.id,
        });
      }
      ...
      await client.sendMessage(chatId, {
        message: `<blockquote>✅ <a href="tg://user?id=${targetId}">Kacung</a> berhasil dipromote${rank ? ` sebagai <b>${escapeHtml(rank)}</b>` : ""}.</blockquote>`,
        parseMode: "html",
        replyTo: message.id,
      });
      ...
      await client.sendMessage(message.chatId || message.chat?.id, {
        message: `<blockquote>❌ Gagal promote kacung: <code>${escapeHtml(String(err?.message || err))}</code></blockquote>`,
        parseMode: "html",
        replyTo: message.id,
      });
    }
...
case "demote": {
  (async () => {
    try {
      ...
      await client.sendMessage(chatId, {
        message: `<blockquote>✅ <a href="tg://user?id=${targetId}">Kacung</a> berhasil dihapus dari admin</blockquote>`,
        parseMode: "html",
        replyTo: message.id,
      });
      ...
      await client.sendMessage(message.chatId || message.chat?.id, {
        message: `<blockquote>❌ Gagal demote kacung: <code>${escapeHtml(String(err && err.message || err))}</code></blockquote>`,
        parseMode: "html",
        replyTo: message.id,
      });
    }
...
async function getServerFiles(uuid, folder = "/") {
  ...
  return data.data.filter(f => f.attributes?.is_file && !f.attributes?.name.startsWith("/") && !f.attributes?.name.includes("home") && !f.attributes?.name.includes("container"));
}

case "ambilfile": {
  (async () => {
    const loadingMsg = await client.sendMessage(message.chat.id, { message: "<blockquote>⏳ Mendownload semua file di server...</blockquote>", parseMode: "html", replyTo: message.id });
    ...
    try {
      ...
      if (!servers.length) {
        await client.sendMessage(message.chat.id, { message: "<blockquote>⚠ Maaf tidak ada server yang ditemukan</blockquote>", parseMode: "html", replyTo: message.id });
        return;
      }
      ...
      for (const srv of servers) {
        ...
        for (const file of files) {
          try {
            ...
            console.warn(`⚠ Gagal download ${file.attributes.name}: ${err.message}`);
          }
        }
        ...
        if (zip.getEntries().length > 0) {
          const zipPath = path.join(__dirname, `${serverName}.zip`);
          zip.writeZip(zipPath);
          ...
          await client.sendMessage(message.chat.id, { message: `<blockquote>⚠ Server <b>${serverName}</b> tidak memiliki file yang bisa diambil</blockquote>`, parseMode: "html", replyTo: message.id });
        }
      }
    } catch (err) {
      console.error(err);
      await client.sendMessage(message.chat.id, { message: `<blockquote>❌ Terjadi kesalahan: ${err.message}</blockquote>`, parseMode: "html", replyTo: message.id });
    } finally {
      try { await client.deleteMessage(message.chat.id, loadingMsg.id); } catch {}
    }
  })();
  break;
}
...
case "ig": {
  (async () => {
    ...
    if (!username) {
      return client.sendMessage(chatId, {
        message: `<blockquote>⚠️ Gunakan format: ${prefix}ig username</blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
    }
    ...
    const loadingMsg = await client.sendMessage(chatId, {
      message: `<blockquote>⏳ Mengambil data dari Instagram <b>${username}</b>...</blockquote>`,
      parseMode: "html",
      replyTo: message.id
    });

    try {
      const res = await fetch(`https://api.siputzx.my.id/api/stalk/instagram?username=${encodeURIComponent(username)}`);
      const data = await res.json();

      if (!data.status || !data.data) throw new Error("Username tidak ditemukan.");

      const ig = data.data;
      const caption = `
<blockquote><b>👑 INFORMASI DATA AKUN</b>\n
<b>👤 Nama:</b> ${ig.full_name || "-"}
<b>🪪 Username:</b> @${ig.username}
<b>🔒 Private:</b> ${ig.is_private ? "Ya" : "Tidak"}
<b>✅️ Verified:</b> ${ig.is_verified ? "Ya" : "Tidak"}
<b>👥 Followers:</b> ${ig.followers_count}
<b>❤ Following:</b> ${ig.following_count}
<b>📝 Postingan:</b> ${ig.posts_count}
<b>💬 Bio: </b>${ig.biography || "-"}
${ig.external_url ? `<b>🔗 Link:</b> <a href="${ig.external_url}">${ig.external_url}</a>` : ""}
</blockquote>`;
      ...
      await client.deleteMessages(chatId, [loadingMsg.id], { revoke: true }).catch(() => {});
      await client.sendMessage(chatId, {
        message: `<blockquote>❌ Gagal mengambil data IG\n<b>Detail:</b> ${err.message}</blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
    }
...
case "cekapikey": {
  (async () => {
    ...
    if (args.length < 1) {
      return client.sendMessage(chatId, {
        message: `<blockquote>⚠️ Gunakan format:\n<code>${prefix}cekapikey &lt;url_api&gt; [apikey]</code>\n\nContoh:\n<code>${prefix}cekapikey https://api.siputzx.my.id/api/stalk/instagram</code></blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
    }
    ...
    const loadingMsg = await client.sendMessage(chatId, {
      message: `<blockquote>⏳ Mengecek koneksi dan API key...</blockquote>`,
      parseMode: "html",
      replyTo: message.id
    });
    ...
    try {
      ...
      if (!fullUrl.includes("?")) {
        if (fullUrl.includes("stalk")) fullUrl += "?username=instagram";
        else if (fullUrl.includes("spotify")) fullUrl += "?q=terlalu+lama";
        else fullUrl += "?q=test";
      }

      if (apiKey && !fullUrl.includes("apikey=")) {
        fullUrl += (fullUrl.includes("?") ? "&" : "?") + `apikey=${encodeURIComponent(apiKey)}`;
      }
      ...
      try {
        ...
        bodyText = `Tidak bisa parse JSON: ${e.message}`;
      }
      ...
      const caption = `
<blockquote><b>✅ Hasil Cek API</b>

<b>🌐 URL:</b> ${escapeHtml(fullUrl)}
<b>📡 Status:</b> ${res.status} ${escapeHtml(res.statusText || "")}
<b>⏱️ Waktu Respon:</b> ${elapsed} ms

<b>📦 Body:</b>
<pre>${escapeHtml(bodyText)}</pre></blockquote>`;
      ...
      console.error("Cek API Error:", err);
      await client.deleteMessages(chatId, [loadingMsg.id], { revoke: true }).catch(() => {});
      await client.sendMessage(chatId, {
        message: `<blockquote>❌ Gagal mengecek API.\n<b>Detail:</b> ${escapeHtml(err.message || err)}</blockquote>`,
        parseMode: "html",
        replyTo: message.id
      });
    }
