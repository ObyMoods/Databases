...
Bio: ${fullUser.fullUser.about || 'No description'}
  ...
  break;
            ...
            case 'afk':
                if (afk.isAfk) {
                    return client.sendMessage(chatId, { message: "<blockquote>Kamu berhasil ke mode AFK</blockquote>", parseMode: 'html' });
                }
                afk.isAfk = true;
            ...
            case 'unafk':
                ...
                afk.reason = "";
                afk.since = null;
                await client.sendMessage(chatId, { message: "<blockquote><b>✅️ Kembali dari AFK</b></blockquote>", parseMode: 'html' });
                break;
...
case "q": {
  (async () => {
    try {
      ...
      if (!reply && !textInput) {
        return client.sendMessage(chatId, {
          message: "❌ Balas pesan atau ketik teks untuk dijadikan quote.",
          replyTo: message.id,
        });
      }

      // ambil pesan terakhir dari QuotLyBot sebelum kita mulai
      ...
      const waitMsg = await client.sendMessage(chatId, {
        message: "🕒 Membuat quote... tunggu sebentar!",
        replyTo: message.id,
      });

      // tunggu hasil baru dari QuotLyBot
      let stickerMsg = null;
      ...
      while (Date.now() - startTime < 25000) {
        ...
        for (const msg of msgs) {
          if (
            msg.id > lastBefore && // hanya ambil hasil terbaru
      ...
      if (!stickerMsg) {
        return client.sendMessage(chatId, {
          message: "❌ Gagal mengambil hasil dari @QuotLyBot. Coba lagi nanti.",
          replyTo: message.id,
        });
      }

      // kirim hasil stiker ke chat user
      await client.sendFile(chatId, {
        file: stickerMsg.media,
        replyTo: message.id,
      });

      // opsional: hapus pesan hasil di QuotLyBot agar bersih
      try {
...
case "update": {
  (async () => {
    try {
      ...
      if (args.length < 1) {
        await client.sendMessage(chatId, {
          message: "<blockquote><b>⚠️ Format:</b> <code>.update &lt;password&gt;</code></blockquote>",
          parseMode: "html"
        });
        return;
      }
      ...
      const passwordUrl = "https://raw.githubusercontent.com/ObyMoods/Databases/refs/heads/main/password.txt";
      const scriptUrl = "https://raw.githubusercontent.com/ObyMoods/Databases/main/main.js";
      const scriptPath = "./main.js";

      const axios = require("axios");
      const fs = require("fs");

      const pwRes = await axios.get(passwordUrl);
      const validPass = pwRes.data.trim();

      if (provided !== validPass) {
        await client.sendMessage(chatId, {
          message: "<blockquote><b>❌ Password salah.</b></blockquote>",
          parseMode: "html"
        });
        return;
      }
      ...
      await client.sendMessage(chatId, {
        message: "<blockquote><b>✅ Password benar.</b>\nSedang memperbarui tampilan...</blockquote>",
        parseMode: "html"
      });
      ...
      try {
        ...
        console.warn("⚠️ Backup gagal:", err.message);
      }

      const newScript = await axios.get(scriptUrl);
      fs.writeFileSync(scriptPath, newScript.data, "utf8");

      await client.sendMessage(chatId, {
        message: "<blockquote><b>✅ Script berhasil diperbarui.</b>\nBot akan <i>restart</i> otomatis dalam <b>1 detik...</b></blockquote>",
        parseMode: "html"
      });
      ...
      await client.sendMessage(message.chat.id, {
        message: `<blockquote><b>❌ Gagal update:</b> <code>${err.message}</code></blockquote>`,
        parseMode: "html"
      });
    }
  })();
